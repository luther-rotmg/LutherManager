...
export function installHeadlessBridge(deps: BridgeDeps): void {
  ...
  Self.getGearMaxHP = () => optional(deps)?.getPlayer()?.maxHPBoost ?? 0;
  Self.getGearMaxMP = () => optional(deps)?.getPlayer()?.maxMPBoost ?? 0;
  Self.getGearAtk = () => optional(deps)?.getPlayer()?.atkBoost ?? 0;
  Self.getGearDef = () => optional(deps)?.getPlayer()?.defBoost ?? 0;
  Self.getGearSpd = () => optional(deps)?.getPlayer()?.spdBoost ?? 0;
  Self.getGearDex = () => optional(deps)?.getPlayer()?.dexBoost ?? 0;
  Self.getGearVit = () => optional(deps)?.getPlayer()?.vitBoost ?? 0;
  Self.getGearWis = () => optional(deps)?.getPlayer()?.wisBoost ?? 0;
  ...
  Walking.enterPortal = (objectId: number) => active(deps).enterPortal(objectId);
  Walking.enterVault = () => active(deps).enterVault();
  Walking.enterPetYard = () => active(deps).enterPetYard();
  ...
  inventory.findItem = (query: number | string) => {
    ...
    const slot = optional(deps)?.findInventoryItem(query);
  ...
  inventory.findItems = (query: number | string) => {
    ...
    return (optional(deps)?.getInventorySlots() ?? [])
      .filter((slot) => slot.objectType === query)
  ...
  inventory.getContainerSlots = (container: InventoryContainer) => optional(deps)?.getContainerSlots(container) ?? [];
  ...
  inventory.getFirstFilledSlot = (container: InventoryContainer) => optional(deps)?.getFirstFilledSlot(container) ?? null;
  inventory.getFirstEmptySlot = (container: InventoryContainer) => optional(deps)?.getFirstEmptySlot(container) ?? null;
  inventory.getContainerItemCount = (container: InventoryContainer) => optional(deps)?.getContainerItemCount(container) ?? 0;
  ...
  inventory.getVault = () => optional(deps)?.getVaultSlots().map((slot) => slot.objectType) ?? [];
  inventory.getPotions = () => optional(deps)?.getPotionVaultSlots().map((slot) => slot.objectType) ?? [];
  inventory.getMaterials = () => optional(deps)?.getContainerSlots('materialVault').map((slot) => slot.objectType) ?? [];
  inventory.getGifts = () => optional(deps)?.getContainerSlots('giftChest').map((slot) => slot.objectType) ?? [];
  inventory.getSeasonalSpoils = () => optional(deps)?.getContainerSlots('spoilsChest').map((slot) => slot.objectType) ?? [];
  inventory.getEntireVault = () => {
    const client = active(deps);
    return {
      capturedAt: Date.parse(client.getVaultContent()?.at ?? '') || Date.now(),
      vault: client.getVaultSlots().map((slot) => slot.objectType),
      material: client.getContainerSlots('materialVault').map((slot) => slot.objectType),
      gift: client.getContainerSlots('giftChest').map((slot) => slot.objectType),
      potion: client.getPotionVaultSlots().map((slot) => slot.objectType),
      seasonalSpoils: client.getContainerSlots('spoilsChest').map((slot) => slot.objectType),
    };
  };
