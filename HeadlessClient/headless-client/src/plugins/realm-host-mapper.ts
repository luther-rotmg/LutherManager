import { ReconnectPacket } from 'realmlib';
...
type MapperState = 'idle' | 'scanningNexus' | 'awaitingReconnect' | 'awaitingRealmMap' | 'returningToNexus' | 'complete';
...
export class RealmHostMapper {
  ...
  private step(client: Client): void {
    ...
    console.log(`[${client.alias}] RealmHostMapper: entering ${next.name} (${next.players}/${next.maxPlayers})`);
    if (!client.enterPortal(next.objectId)) {
      this.state = 'scanningNexus';
    }
  }
